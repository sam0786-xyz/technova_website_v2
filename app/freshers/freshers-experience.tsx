...
const days = [
  { no: 1, title: "Welcome", verb: "Settle in. Find your bearings.", color: "#ff7043", sessions: [["09:00", "Welcome session", "Main Auditorium", "Live"], ["10:30", "Faculty interaction", "Block 2, Seminar Hall", "Upcoming"], ["12:00", "Campus experience", "Reporting Desk", "Upcoming"], ["14:00", "Student activities", "Central Lawn", "Upcoming"]] },
  { no: 2, title: "Explore", verb: "Make the campus feel like yours.", color: "#47b7ff", sessions: [["09:30", "Campus walk", "Gate 1", "Upcoming"], ["11:30", "Library & learning", "Central Library", "Upcoming"], ["14:00", "Find what you need", "Student Services", "Upcoming"]] },
  { no: 3, title: "Connect", verb: "Start finding your people.", color: "#de7dff", sessions: [["10:00", "Meet your cohort", "Block 2 Atrium", "Upcoming"], ["12:00", "Student community hour", "Central Lawn", "Upcoming"], ["15:00", "Conversation circles", "Seminar Hall", "Upcoming"]] },
  { no: 4, title: "Discover", verb: "Find things worth showing up for.", color: "#efbf45", sessions: [["10:00", "Beyond the classroom", "Main Auditorium", "Upcoming"], ["12:30", "Community discovery", "Experience Hall", "Upcoming"], ["15:00", "Build your next step", "DevSpace", "Upcoming"]] },
  { no: 5, title: "University", verb: "Learn how the bigger picture works.", color: "#6dd98c", sessions: [["10:00", "University systems", "Main Auditorium", "Upcoming"], ["12:00", "Support that exists", "Student Services", "Upcoming"], ["14:30", "Careers begin here", "Block 1", "Upcoming"]] },
  { no: 6, title: "Launch", verb: "Turn six days into momentum.", color: "#fd99bb", sessions: [["10:00", "Your next chapter", "Main Auditorium", "Upcoming"], ["12:00", "Final reflection", "Central Lawn", "Upcoming"], ["14:00", "Passport ceremony", "Main Auditorium", "Upcoming"]] },
];
...
function Start({ onHelp, profile }: { onHelp: () => void; profile?: Profile }) {
  const [step, setStep] = useState(0); const [name, setName] = useState(profile?.name || ""); const [school, setSchool] = useState(profile?.school || "School of Engineering & Technology"); const [programme, setProgramme] = useState(profile?.programme || "B.Tech — Computer Science"); const [chosen, setChosen] = useState<string[]>(profile?.interests || ["AI", "Coding"]);
  ...
  return <Shell minimal onHelp={onHelp} profile={profile}><main className="f-onboarding">
    ...
    {step === 0 && <section className="f-setup f-reveal"><p className="f-kicker">Make it yours</p><h1>First things<br/>first.</h1><p className="f-lede">A few details, then we’ll shape your first week around you.</p><div className="f-fields"><label>Your name<input value={name} onChange={e => setName(e.target.value)} placeholder="What should we call you?" autoFocus /></label><label>School<select value={school} onChange={e => setSchool(e.target.value)}><option>School of Engineering & Technology</option><option>School of Business Studies</option><option>School of Design</option><option>School of Law</option></select></label><label>Programme<input value={programme} onChange={e => setProgramme(e.target.value)} /></label></div><Button onClick={next}>Keep going</Button></section>}
...
function Orientation({ onHelp, profile }: { onHelp: () => void; profile?: Profile }) {
  ...
  return <Shell onHelp={onHelp} profile={profile}><main className="f-page f-orientation">
    <div className="f-sectionhead"><div>{source("official")}<p className="f-kicker">Your orientation</p><h1>Day 01 <span>/ 06</span></h1></div><div className="f-progress"><span>THE JOURNEY</span><div><i style={{width:"17%"}}/></div><b>1 of 6</b></div></div>
    <section className="f-today"><div className="f-today-intro"><p className="f-kicker">Today</p><h2>Welcome.</h2><p>Your only job today: arrive, look around, and ask the questions you have.</p><Button href="/freshers/day/1">Open day one</Button></div><div className="f-sessions">{today.sessions.map((s, i) => <Link href="/freshers/day/1" className="f-session" key={s[1]}><time>{s[0]}</time><div><span className={i === 0 ? "f-live" : "f-status"}>{i === 0 ? "● Live now" : "Upcoming"}</span><h3>{s[1]}</h3><p><MapPin size={13}/>{s[2]}</p></div><ChevronRight size={19}/></Link>)}</div></section>
...
function Day({ onHelp, profile, no = 1 }: { onHelp: () => void; profile?: Profile; no?: number }) {
  ...
  return <Shell onHelp={onHelp} profile={profile}><main className="f-page f-day" style={{"--day-color": d.color} as React.CSSProperties}>
    <div className="f-dayhero"><Link href="/freshers/orientation" className="f-back">← All six days</Link><p>DAY 0{d.no} / 06</p><h1>{d.title}<i>.</i></h1><span>{d.verb}</span></div>
    <div className="f-daybody"><section><p className="f-kicker">Where you need to be</p><div className="f-sessions f-sessions--detail">{d.sessions.map((s, i) => <article key={s[1]} className={open === i ? "f-session-detail f-session-detail--open" : "f-session-detail"}><button onClick={() => setOpen(open === i ? null : i)}><time>{s[0]}</time><div><span className={i === 0 && no === 1 ? "f-live" : "f-status"}>{i === 0 && no === 1 ? "● Live now" : "Starts in 24 min"}</span><h3>{s[1]}</h3><p><MapPin size={13}/>{s[2]}</p></div><ChevronRight size={19}/></button>{open === i && <div className="f-session-extra"><p>A focused session designed to give you the information you need now — nothing more, nothing less.</p><div><button><CalendarDays size={15}/> Add to calendar</button><Link href="/freshers/campus"><Compass size={15}/> Get directions</Link></div><small>Last updated today at 08:42 AM</small></div>}</article>)}</div></section>
