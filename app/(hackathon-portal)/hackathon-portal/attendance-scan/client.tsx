    ...
    return (
        <div className="min-h-screen bg-black text-white p-4">
            ...
            <main className="max-w-md mx-auto space-y-4">
                {/* Checkpoint Selector */}
                <div className="bg-violet-500/10 border border-violet-500/20 rounded-none neo-border p-3">
                    ...
                    <div className="relative">
                        <select
                            ...
                            className="w-full bg-black/60 border border-white/20 rounded-none px-4 py-2.5 text-white text-sm font-medium appearance-none focus:outline-none focus:border-violet-500 cursor-pointer"
                        >
                            {checkpoints.map(cp => (
                ...
                <div className="flex bg-[#03030F]/5 p-1 rounded-none neo-border">
                    <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-none text-xs font-medium transition-colors ${!showSearch ? 'bg-violet-600 text-white' : 'text-white/40 font-mono hover:text-white'}`}
                    >
                ...
                {showSearch ? (
                    <div className="space-y-3">
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full bg-[#03030F]/5 border border-white/10 rounded-none neo-border pl-10 pr-10 py-3 text-white text-sm focus:outline-none focus:border-violet-500"
                                autoFocus
                            />
                            {searchQuery && (
                        ...
                        {loadingAttendees ? (
                            <div className="text-center py-6">
                                ...
                                <p className="text-white/50 font-mono tracking-wider text-xs">Loading attendees...</p>
                            </div>
                            ...
                            <div className="space-y-2">
                                {paginatedAttendees.map((a: any) => {
                                    ...
                                    return (
                                        <div key={a.id} className="flex items-center justify-between p-3 rounded-none neo-border bg-[#03030F]/5 border border-white/10">
                                            <div className="flex-1 min-w-0 mr-3">
                                                ...
                                                <div className="flex gap-1 mt-1">
                                                    {checkpoints.map(cp => (
                                                        <span key={cp} className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${hasScannedCheckpoint(a, cp) ? 'bg-emerald-500/20 text-emerald-400' : 'bg-[#03030F]/5 text-white/70'}`}>
                                                            {cp}
                                                        </span>
                                                    ))}
                                                </div>
                                            ...
                                            <button
                                                ...
                                                className={`px-3 py-1.5 rounded-none text-xs font-bold transition-all disabled:opacity-40 ${alreadyScanned ? 'bg-emerald-600/20 text-emerald-400 cursor-not-allowed' : 'bg-violet-600 hover:bg-violet-500 text-white'}`}
                                            >
                                    ...
                                    {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                                        ...
                                        return (
                                            <button key={pageNum} onClick={() => setCurrentPage(pageNum)}
                                                className={`w-8 h-8 rounded-none text-xs font-medium transition-all ${currentPage === pageNum ? 'bg-violet-600 text-white' : 'bg-[#03030F]/5 text-white/40 font-mono hover:text-white'}`}>
                                                {pageNum}
                                            </button>
                                        );
                                    })}
                    ...
                    <>
                        ...
                        <div className="relative w-full aspect-square bg-zinc-900 rounded-none neo-border overflow-hidden border border-white/10 flex flex-col items-center justify-center">
                            ...
                            {scanResult && (
                                <div className={`absolute inset-0 z-20 flex flex-col items-center justify-center p-6 text-center backdrop-blur-md ${scanResult === 'success' ? 'bg-emerald-500/90' :
                                    ...
                                    {attendeeName && <p className="text-xl font-bold text-white mb-1">{attendeeName}</p>}
                                    {attendeeDetails && <p className="text-sm text-white/80 mb-1">{attendeeDetails}</p>}
