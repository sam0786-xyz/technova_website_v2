...
export default function EvaluatorDashboardClient({ initialTeams, evaluationOpen = true, evaluationRounds = 2, evaluatorToken }: { initialTeams: any[], evaluationOpen?: boolean, evaluationRounds?: number, evaluatorToken?: string }) {
    ...
    const renderScoreInput = (label: string, field: string, description: string) => (
        <div key={field} className="bg-[#03030F]/5 border border-white/10 rounded-none neo-border p-4">
            ...
            <input
                ...
                className="w-full bg-black/50 border border-white/20 rounded-none px-4 py-3 text-center text-white text-lg font-bold focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500/50 transition-all"
                placeholder="0.0 - 10.0"
            />
            <p className="text-xs text-white/50 font-mono tracking-wider mt-2">{description}</p>
        </div>
    );
    ...
    return (
        <div className="space-y-6">
            {/* Round Selector */}
            <div className={`flex bg-[#03030F]/5 border border-white/10 rounded-none neo-border p-1 w-full flex-wrap md:flex-nowrap mx-auto mb-8`}>
                {Array.from({ length: evaluationRounds }, (_, i) => i + 1).map((r) => (
                    <button
                        key={r}
                        onClick={() => setRound(r)}
                        className={`flex-1 min-w-[120px] md:px-8 py-2.5 rounded-none text-sm font-medium transition-all ${round === r ? 'bg-amber-500 text-black shadow-lg shadow-amber-500/20' : 'text-white/40 font-mono hover:text-white hover:bg-[#03030F]/5'}`}
                    >
                        Round {r}
                    </button>
                ))}
            ...
            <div className="flex flex-wrap gap-2 justify-center mb-4">
                {THEMES.map(theme => (
                    <button
                        ...
                        className={`px-4 py-2 rounded-none text-xs font-bold uppercase tracking-wider transition-all border ${selectedTheme === theme.code
                            ? 'bg-amber-500 text-black border-amber-500 shadow-lg shadow-amber-500/20'
                            : 'bg-[#03030F]/5 text-white/40 font-mono border-white/10 hover:bg-[#03030F]/10 hover:text-white'
                            }`}
                    >
                        {theme.label}
                        <span className={`ml-1.5 px-1.5 py-0.5 rounded text-[10px] ${selectedTheme === theme.code ? 'bg-black/20 text-black' : 'bg-[#03030F]/10 text-white/50 font-mono tracking-wider'
                            }`}>
            ...
            {uniqueColleges.length > 0 && (
                <div className="flex justify-center mb-4">
                    <select
                        ...
                        className="bg-[#03030F]/5 border border-white/10 rounded-none neo-border px-4 py-2.5 text-sm text-white focus:outline-none focus:border-amber-500 appearance-none cursor-pointer min-w-[200px]"
                    >
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between mb-8">
                <div className="relative w-full md:w-96">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-white/50 font-mono tracking-wider" />
                    <input
                        type="text"
                        placeholder="Search by team name, ID, or project..."
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        className="w-full bg-[#03030F]/5 border border-white/10 rounded-none neo-border pl-10 pr-4 py-3 focus:outline-none focus:border-amber-500 text-white transition-colors"
                    />
                </div>
                <div className="flex gap-4 text-sm font-medium">
                    <div className="flex items-center gap-2">
                        <div className="w-3 h-3 rounded-full bg-amber-500" />
                        <span className="text-white/40 font-mono">To Evaluate ({filteredTeams.filter(t => !t.has_evaluated).length})</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <div className="w-3 h-3 rounded-full bg-emerald-500" />
                        <span className="text-white/40 font-mono">Completed ({filteredTeams.filter(t => t.has_evaluated).length})</span>
                    </div>
                </div>
            </div>
            ...
            <div className="grid gap-4">
                {loadingTeams ? (
                    <div className="text-center py-12 text-white/50 font-mono tracking-wider border border-dashed border-white/10 rounded-none neo-border flex flex-col items-center justify-center">
                    ...
                    <div className="text-center py-12 text-white/50 font-mono tracking-wider border border-dashed border-white/10 rounded-none neo-border">
                        No teams found{selectedTheme !== 'ALL' ? ` for theme ${selectedTheme}` : ''}{searchQuery ? ` matching "${searchQuery}"` : ''}.
                    </div>
                ) : (
                    <>
                        <div className="overflow-x-auto rounded-none neo-border border border-white/10 bg-[#03030F]/5 shadow-2xl">
                            <table className="w-full text-sm text-left text-white/40 font-mono">
                                <thead className="text-xs text-white/40 font-mono uppercase bg-black/40 border-b border-white/10">
                                    <tr>
                                            ...
                                            <Fragment key={team.id}>
                                                ...
                                                >
                                                    ...
                                                    <td className="px-4 py-4 text-right whitespace-nowrap">
                                                        {!evaluationOpen ? (
                                                            <span className="text-white/40 font-mono font-bold flex items-center justify-end gap-1.5"><Lock className="w-4 h-4" /> Final: {team.total_score || 0}</span>
                                                        ) : team.has_evaluated ? (
                                                ...
                                                {isExpanded && !evaluationOpen && (
                                                    <tr>
                                                        <td colSpan={9} className="p-0 border-b border-white/20/20 shadow-inner">
                                                            <div className="p-6 md:p-8 bg-black/40">
                                                                <div className="mb-6 bg-red-500/10 border border-red-500/20 p-5 rounded-none neo-border shadow-lg flex items-start gap-4 text-red-400">
                                                                ...
                                                                {loadingReviews ? (
                                                                    <div className="text-center py-8">
                                                                        ...
                                                                        <p className="text-white/40 font-mono text-sm">Loading peer reviews...</p>
                                                                    </div>
                                                                ) : peerReviews.length === 0 ? (
                                                                    <div className="text-center py-8 border border-dashed border-white/10 rounded-none neo-border">
                                                                        <p className="text-white/50 font-mono tracking-wider text-sm italic">No evaluations were submitted for this team.</p>
                                                                    </div>
                                                                ) : (
                                                                    <div className="space-y-4">
                                                                        {peerReviews.map((rev: any) => (
                                                                            <div key={rev.id} className="bg-[#03030F]/5 border border-white/10 rounded-none neo-border p-5 shadow">
                                                                                <div className="flex justify-between items-start mb-4 border-b border-white/5 pb-4">
                                                                                    <div>
                                                                                        <h5 className="font-bold text-white text-lg">{rev.hackathon_evaluators?.name || 'Unknown Evaluator'}</h5>
                                                                                        <p className="text-xs text-white/50 font-mono tracking-wider">{new Date(rev.created_at).toLocaleString()}</p>
                                                                                    </div>
                                                                                    <div className="flex flex-col items-end">
                                                                                        <div className="text-2xl font-black text-amber-500 bg-amber-500/10 px-4 py-1.5 rounded-none neo-border border border-amber-500/20">
                                                                                ...
                                                                                <div className="grid grid-cols-2 md:grid-cols-5 gap-2 mb-4 text-xs font-mono text-center">
                                                                                    <div className="bg-black/30 p-2 rounded border border-white/5"><span className="block text-white/50 font-mono tracking-wider mb-1">INNOV</span><span className="text-emerald-400 font-bold text-base">{rev.score_innovation}</span><span className="text-white/70">/5</span></div>
                                                                                    <div className="bg-black/30 p-2 rounded border border-white/5"><span className="block text-white/50 font-mono tracking-wider mb-1">FEAS</span><span className="text-emerald-400 font-bold text-base">{rev.score_feasibility}</span><span className="text-white/70">/5</span></div>
                                                                                    <div className="bg-black/30 p-2 rounded border border-white/5"><span className="block text-white/50 font-mono tracking-wider mb-1">IMPACT</span><span className="text-emerald-400 font-bold text-base">{rev.score_impact}</span><span className="text-white/70">/5</span></div>
                                                                                    <div className="bg-black/30 p-2 rounded border border-white/5"><span className="block text-white/50 font-mono tracking-wider mb-1">UX</span><span className="text-emerald-400 font-bold text-base">{rev.score_ux}</span><span className="text-white/70">/5</span></div>
                                                                                    <div className="bg-black/30 p-2 rounded border border-white/5"><span className="block text-white/50 font-mono tracking-wider mb-1">PRES</span><span className="text-emerald-400 font-bold text-base">{rev.score_presentation}</span><span className="text-white/70">/5</span></div>
                                                                                </div>
                                                                                {rev.feedback && (
                                                                                    <div className="bg-black/30 rounded-none p-4 text-sm text-gray-300 border-l-2 border-emerald-500">
                                                    ...
                                                    <tr>
                                                        <td colSpan={9} className="p-0 border-b border-amber-500/20 shadow-inner">
                                                            <div className="p-6 md:p-8 bg-black/40">
                                                                ...
                                                                <div className="mb-6 bg-[#03030F]/5 border border-white/10 p-5 rounded-none neo-border shadow-lg">
                                                                ...
                                                                <div className="mb-8 text-sm text-white/40 font-mono bg-amber-500/10 border border-amber-500/20 rounded-none neo-border p-4 flex flex-col items-start gap-4 shadow-lg">
                                                                ...
                                                                <div className="space-y-3 mb-6">
                                                                    ...
                                                                    <textarea
                                                                        ...
                                                                        className="w-full bg-[#03030F]/5 border border-white/10 rounded-none neo-border p-4 focus:outline-none focus:border-amber-500 text-white resize-none h-24"
                                                                    />
                                                                </div>

                                                                <div className="flex items-center justify-between pt-4 border-t border-white/10">
                                                                    <div className="text-2xl font-black text-white">
                                                                        Total: <span className="text-amber-500">{((Number(scores.innovation)*2.5) + (Number(scores.feasibility)*2) + (Number(scores.impact)*2) + (Number(scores.ux)*1.5) + (Number(scores.presentation)*2)).toFixed(1)}</span><span className="text-white/50 font-mono tracking-wider text-lg">/100</span>
                                                                    </div>

                                                                    <button
                                                                        onClick={() => handleSubmitClick(team.id)}
                                                                        disabled={submitting}
                                                                        className="bg-amber-600 hover:bg-amber-500 text-white font-medium py-2.5 px-8 rounded-none neo-border transition-all disabled:opacity-50 flex items-center gap-2"
                                                                    >
                                                                        {submitting ? (
                                                                ...
                                                                {message && (
                                                                    <div className={`mt-4 p-4 rounded-none neo-border flex items-start gap-3 ${message.type === 'error' ? 'bg-red-500/10 border border-red-500/20 text-red-400' : 'bg-emerald-500/10 border border-emerald-500/20 text-emerald-400'}`}>
                                                    ...
                                                    <tr>
                                                        <td colSpan={9} className="p-0 border-b border-emerald-500/20 shadow-inner">
                                                            <div className="p-8 bg-black/40 text-center">
                                                                ...
                                                                <p className="text-white/40 font-mono mb-6">You gave this team a total score of <span className="text-emerald-400 font-bold">{team.my_score}</span>. Thank you for your review.</p>
                                                                
                                                                {team.edit_requested ? (
                                                                    <div className="inline-flex items-center gap-2 px-4 py-2 bg-amber-500/10 text-amber-500 rounded-none text-sm font-medium border border-amber-500/20">
                                                                    ...
                                                                    <button
                                                                        ...
                                                                        disabled={submitting}
                                                                        className="inline-flex items-center gap-2 px-4 py-2 bg-[#03030F]/5 hover:bg-[#03030F]/10 text-white rounded-none text-sm font-medium transition-colors border border-white/10"
                                                                    >
                            ...
                            <div className="flex items-center justify-between pt-4">
                                ...
                                <div className="flex items-center gap-2">
                                    <button
                                        ...
                                        className="p-2 rounded-none bg-[#03030F]/5 border border-white/10 text-white/40 font-mono hover:bg-[#03030F]/10 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                                    >
                                    ...
                                    {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
                                        <button
                                            ...
                                            className={`w-9 h-9 rounded-none text-sm font-medium transition-all ${currentPage === page
                                                ? 'bg-amber-500 text-black shadow-lg shadow-amber-500/20'
                                                : 'bg-[#03030F]/5 text-white/40 font-mono border border-white/10 hover:bg-[#03030F]/10'
                                                }`}
                                        >
                                            {page}
                                        </button>
            ...
            {confirmModalData && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
                    <div className="bg-zinc-900 border border-white/20 rounded-none neo-border p-6 w-full max-w-md shadow-2xl relative">
                        <h3 className="text-xl font-bold text-white mb-4">Confirm Submission</h3>
                        <p className="text-white/40 font-mono text-sm mb-6">Are you sure you want to submit? Please review the breakdown below:</p>
                        
                        <div className="space-y-3 bg-black/50 p-4 rounded-none neo-border border border-white/5 mb-6">
                            <div className="flex justify-between items-center">
                                <span className="text-white/40 font-mono text-sm">Innovation (25%)</span>
                                <span className="text-white font-semibold">{confirmModalData.scores.innovation} / 10</span>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-white/40 font-mono text-sm">Feasibility (20%)</span>
                                <span className="text-white font-semibold">{confirmModalData.scores.feasibility} / 10</span>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-white/40 font-mono text-sm">Impact (20%)</span>
                                <span className="text-white font-semibold">{confirmModalData.scores.impact} / 10</span>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-white/40 font-mono text-sm">UX (15%)</span>
                                <span className="text-white font-semibold">{confirmModalData.scores.ux} / 10</span>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-white/40 font-mono text-sm">Presentation (20%)</span>
                        ...
                        <div className="flex gap-3 justify-end">
                            <button 
                                ...
                                disabled={submitting}
                                className="px-5 py-2.5 rounded-none neo-border border border-white/10 text-gray-300 hover:bg-[#03030F]/5 transition-colors font-medium text-sm"
                            >
                                Cancel
                            </button>
                            <button 
                                ...
                                disabled={submitting}
                                className="px-5 py-2.5 rounded-none neo-border bg-amber-500 hover:bg-amber-400 text-black transition-colors font-bold text-sm min-w-[120px]"
                            >
